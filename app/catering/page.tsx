import CateringClient from "./CateringClient";
import type { Metadata } from "next";

export const metadata: Metadata = {
    title: "Oferta Cateringowa Lublin – Catering Zielony Talerzyk",
    description: "Szukasz sprawdzonego cateringu w Lublinie? Zielony Talerzyk oferuje profesjonalny catering okolicznościowy, finger food na eventy firmowe, chrzciny, komunie i przyjęcia. Sprawdź naszą ofertę!",
    keywords: [
        "Catering okolicznościowy Lublin",
        "Partybox Lublin",
        "Grill imprezy plenerowe Lublin",
        "Fingerfood Lublin",
        "Catering serwowany Lublin",
        "Oferta cateringowa Lublin",
        "nasz catering Lublin",
        "catering na urodziny Lublin",
        "catering na przyjęcia Lublin",
        "catering dla firm Lublin",
        "catering na komunię Lublin",
        "catering na chrzciny Lublin"
    ],
};

export default function CateringPage() {
    return (
        <main className="min-h-screen relative overflow-hidden">
            {/* Consistent gradient background */}
            <div className="absolute inset-0 -z-10">
                <div className="absolute inset-0 bg-gradient-to-br from-[#f0f9f4] via-[#ffffff] to-[#f5fef0]" />
                <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-[rgba(196,214,0,0.15)] via-transparent to-transparent" />
                <div className="absolute bottom-0 right-0 w-full h-full bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] from-[rgba(0,138,75,0.12)] via-transparent to-transparent" />
            </div>

            <CateringClient />
        </main>
    );
}
